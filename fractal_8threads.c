...
static void *mandelbrot_thread(void *args)
{
	...
	for (i = 0; i < width; i++) {
		for (j = data->start_j; j < data->end_j; j++) {
			...
			if (ii == 1500) {
				data->pixmap[j + i * width] = 0;
			}
			else {
				int c = (int)((ii / 32.0f) * 256.0f);
				data->pixmap[j + i * width] = pal[c%256];
			}
		}
	}
}

static void init_threads_mandelbrot(unsigned int *pixmap)
{
	int i;
	...
	for(i = 0; i < NR_OF_THREADS; i++) {
		...
		t_data[i].pixmap = pixmap;
		int t0 = pthread_create(&threads[i], NULL, mandelbrot_thread, &t_data[i]);
	}
}

void writetga(unsigned int *pixmap)
{
	FILE *f;
	int i, j;
	char buffer[50];
	f = fopen("fracout.tga", "wb");
...
{
	unsigned int* pixmap = malloc(width * height * sizeof(int));

	init_threads_mandelbrot(pixmap); 
	join_threads();
